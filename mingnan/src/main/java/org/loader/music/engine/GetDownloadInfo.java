...
public class GetDownloadInfo {
	...
	private void getLrcUrl(final int position, final String url) {
		...
		/*	Document doc = Jsoup
			...
			Message msg = mHandler.obtainMessage(GET_LRC_SUCCESS, url);
			msg.arg1 = position;
			msg.sendToTarget();
		} catch (Exception e) {
	...
	private void getDownloadUrl(final int position, final String url) {
...
//			String result = targetElements.get(0).attr("href");
			Message msg = mHandler.obtainMessage(GET_MUSIC_SUCCESS, url);
			msg.arg1 = position;
			msg.sendToTarget();
		} catch (Exception e) {
